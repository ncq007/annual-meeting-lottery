  ...
  $.fn.lucky = function (opt) {
    ...
    var stepFun = function () {
      ...
      for (var i = 0; i < elements; i++) {
        ...
        if (!!settings.data[i]) {
          $(element).css('background-image', 'url(' + settings.data[i].avatar_url + ')');
        } else {
          index >= (settings.data.length - 1) ? index = 1 : ++index;
          // $(element).css('background-image', 'url("../../images/tx.png")');
          $(element).css('background-image', 'url(' + settings.data[index].avatar_url + ')');
        }
        ...
        setTimeout(function (element) {
          var random = Math.floor(Math.random() * 3) + 20; // 调转速
          tick(function () {
            if (!M.isStop) {
      ...
      tick(function () {
        thisElement.translateZ >= 200 ? thisElement.translateZ = 0 : thisElement.translateZ += 20;
      })
    }
    ...
    var randomFun = function () {
      M.timer = setInterval(function () {
        ...
        $('.js_current_dom').css({
          'background-image': 'url(' + settings.data[randomNum].avatar_url + ')'
        })
      }, 100)
    }
    //停止运动
